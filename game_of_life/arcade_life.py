...
class LifeGame:
	def __init__(self):
		...
		self.__root.protocol('WM_DELETE_WINDOW', self._close_window)
		...
		self.pause_play = tk.Button(self.frame_props, text='Play/Pause Simulation', bg='red', command=self._live)
		...
		for i in range(self.__x):
			for j in range(self.__y):
				button = tk.Button(self.frame_grids, height=1, width=2,
								   command=lambda x=i, y=j: self._change_n_update(x, y))
	...
	def _make_universe(self, new_universe=False):
		...
		if not self.universe_init:
			...
			tk.Button(dialog, text='submit', command=lambda: self._update_grid(row.get(), col.get())).grid(row=2,
																										   column=0)
